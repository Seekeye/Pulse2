      ...
      <div className="relative">
        ...
        <div className={`absolute bottom-16 right-0 transition-all duration-300 ${
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 p-4 min-w-[200px]">
            ...
            {/* Navigation Links */}
            <nav className="space-y-2 mb-4 pb-4 border-b border-gray-100">
              {menuItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => onTabChange(item.id)}
                  className={`flex items-center w-full p-2 rounded-lg text-sm font-medium transition-colors duration-200
                    ${activeTab === item.id ? `bg-${item.color}-50 text-${item.color}-600` : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900'}`}
                >
                  <item.icon className={`w-5 h-5 mr-3 ${activeTab === item.id ? `text-${item.color}-500` : 'text-gray-400'}`} />
                  {item.name}
                </button>
              ))}
            </nav>
        ...
        <button 
          className={`relative w-14 h-14 rounded-full shadow-lg flex items-center justify-center 
            ...
            hover:shadow-xl transition-all duration-300 transform ${isExpanded ? 'rotate-45' : ''}`}
